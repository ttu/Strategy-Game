...
{
    public class ObjectSystem : BaseSystem 
    {
        // Singleton :( All states share same ObjectManager
        private static ObjectSystem _instance = new ObjectSystem();
        ...
        private ObjectSystem()
        {
            SysType = SystemType.ObjectSystem;
        }
        ...
        {
            // TODO: Should query components and update those in order
            List<GameObject> qos = _quadTree.Query(_gameAreaRectangle);

            List<BaseComponent> components = createComponentList(qos);

            foreach (BaseComponent comp in components)
            {
                comp.Update(elapsedTime, _gamespeed);
            }

            //foreach (GameObject gObject in qos)
            //{
            //    if (gObject.BlockMove) // Still in game
        ...
        private List<BaseComponent> createComponentList(List<GameObject> qos)
        {
            List<BaseComponent> retVal = new List<BaseComponent>();

            foreach (Type componentType in ComponentManager.ComponentExecuteOrder)
            {
                foreach (GameObject gObject in qos)
                {
                    if (gObject is Character) // Get rid of character class
                    {
                        BaseComponent comp = ((Character)gObject).TryGetComponent(componentType);
